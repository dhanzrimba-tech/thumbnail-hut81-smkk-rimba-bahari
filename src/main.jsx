...
async function resizeFileForPhone(file, maxSide = 1280) {
  ...
  try {
    ...
    return await new Promise((resolve, reject) => {
      canvas.toBlob(
        (blob) => (blob ? resolve(blob) : reject(new Error("Gagal mengecilkan foto"))),
        "image/jpeg",
        0.88
      );
    });
...
async function makeThumbnail(file, setProgress) {
  const smallFile = await resizeFileForPhone(file, 1280);
  const sourceUrl = URL.createObjectURL(smallFile);
...
function App() {
  ...
  async function handleFile(e) {
    ...
    try {
      const result = await makeThumbnail(file, setMessage);
      setPreview(result);
      setMessage("Berhasil! Thumbnail siap di-download.");
    } catch (err) {
      console.error(err);
      setError(
  ...
  return (
    <main className="page">
      <section className="app">
        <div className="intro">
          ...
          <div className="privacy">
            <ShieldCheck />
            <span>
              Foto diproses langsung di browser. Foto tidak dikirim ke
              server aplikasi.
            </span>
          </div>
      ...
      <section className="steps">
        {[
          [ImagePlus, "Pilih Foto", "Pilih foto dari kamera atau galeri"],
